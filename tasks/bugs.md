...
**DONE.** The handler body is wrapped in try/catch and roads come from `LineFeatures.loadAllRoads()`, so there is no fetch of its own left to check.
...
**DONE.** The code now lives in `js/territory-controller.js`. A null `shadowExclRu` sets a `TerritoryStatus` message (`shadow` key) under the DeepState toggles, cleared on the next render.
...
**DONE.** Results, hints and errors are written to `#road-km-shadow-stats` / `#road-km-diff-stats` under each button.
