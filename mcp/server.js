...
const TOOLS = [
    ...
    },
    {
        ...
        inputSchema: obj({
            ...
            exposure_tolerance: { type: 'number', description: "0-100. Defaults to the profile's own stance (25 for infiltration, 55 for a DRG). How much open ground the group will accept: 0 never leaves cover, 100 ignores distance from cover entirely. Independent of this, open ground far from any settlement is discounted for both profiles — a field in the middle of nowhere is barely watched." },
            ...
            profile: { type: 'string', enum: ['infiltration', 'drg'], description: "Who is moving, which changes what the route wants. 'infiltration' (default) goes village to village: settlements are cheap because they give near-continuous cover and somewhere to halt, rest and accumulate, and their approaches are cheap too. 'drg' is a small group going to a point: it treats built-up ground as a hazard, treats the watched approaches to a village as worse, trades concealment for directness, and moves at 2.5 km/h instead of 1.2." },
    ...
    // ── units ────────────────────────────────────────────────────────────────
    {
        ...
        inputSchema: obj({
            ...
            echelon: { type: 'string', enum: draw.ECHELONS, description: "Formation level. Sets the marker size \u2014 an army outranks a corps outranks a division outranks a brigade, and so on down \u2014 and draws the standard tick marks above the plate (XXXX, XXX, XX, X, III, II, I, \u2022\u2022\u2022). Aliases such as 'CAA', 'MRD', 'bde', 'bn' are accepted. If omitted it is inferred from the unit name where possible; the result says which." },
            echelon_mark: { type: 'boolean', description: 'Draw the echelon tick marks above the plate. Default true; set false to size by echelon without the bar.' },
            size: { type: 'number', description: 'Plate size in pixels, overriding the echelon. Default 30 when no echelon is given.' },
            label_size: { type: 'number' },
